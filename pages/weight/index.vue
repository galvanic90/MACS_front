...
const headers = ref([
  ...
  { title: "Peso inicial", key: "initialWeight" },
  { title: "Peso final", key: "finalWeight" },
...
const handleDelete = async (id) => {
  try {
    await fetcher(`/category-weight/${id}`, {
      method: 'DELETE'
    });
...
const saveItem = async () => {
  ...
  try {
    if(isEditMode.value && categoriesForm.value.id){
      await fetcher(`/category-weight/${categoriesForm.value.id}`, {
        method: 'PUT',
        body: categoriesForm.value
      })
    } else{
  
      await fetcher('/category-weight', {
        method: 'POST',
        body: categoriesForm.value
      });
    }
    ...
    <v-card>
      ...
      <v-card-text>
        <v-form ref="form">
            ...
            <v-text-field v-model="categoriesForm.initialWeight" label="Peso inicial"></v-text-field>
            <v-text-field v-model="categoriesForm.finalWeight" label="Peso final"></v-text-field>
        </v-form>
